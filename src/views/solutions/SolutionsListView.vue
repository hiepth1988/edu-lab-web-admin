...
<template>
  <div>
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold text-slate-900">Solutions</h1>
      <RouterLink
        to="/solutions/new"
    ...
    <div class="mt-6 bg-white rounded-xl border border-slate-200 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-for="row in solutions" v-else :key="row.id">
            ...
            <td class="px-4 py-3 text-right space-x-3">
              <RouterLink :to="`/solutions/${row.id}`" class="text-slate-600 hover:text-slate-900">Sửa</RouterLink>
